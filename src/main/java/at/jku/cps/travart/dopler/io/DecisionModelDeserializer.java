...
public class DecisionModelDeserializer implements IDeserializer<IDecisionModel> {
    ...
    private DecisionModel deserializeFromReaders(final ReaderFactoryOperator readerFactory)
                ...
                if (!cardinality.isEmpty()) {
                    ...
                    if (!(decision instanceof EnumerationDecision) || !(values.length == 2)) {
                        throw new NotSupportedVariabilityTypeException(
                                String.format(CARDINALITY_NOT_SUPPORTED_ERROR, cardinality,
    ...
    @Override
    public IDecisionModel deserializeFromFile(final Path path)
            throws IOException, NotSupportedVariabilityTypeException {
    ...
    @Override
    public IDecisionModel deserialize(final String serial, final Format format)
