...
public class ValidConfigFinder {

    private static final Pattern SPLIT_PATTERN = Pattern.compile("[()]");

    private final Z3Runner z3Runner = new Z3Runner();
    private final Z3OutputParser z3OutputParser = new Z3OutputParser();

    /**
     * How it works:
     * <p>
     * The dopler model is transformed into a smt stream. This stream is given to the z3 sat solver and checked for
     * satisfiability. If it is satisfiable, then the z3 sat solver returns a single config. This config is saved. After
     * that a new call to the z3 sat solver is prepared. This time an assertion is added so that the sat solver cannot
     * produce the same config again. These steps are repeated until the sat solver does not find a new valid config.
     *
     * @param dopler
     * @param maxAmountOfConfigs
     *
     * @return
     */
